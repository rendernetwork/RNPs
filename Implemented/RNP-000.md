...
Members should reach out to an admin or moderator on the Render Network Team to discuss an improvement to the protocol. The Team will help direct if this is a good candidate for an RNP or a Grant. Grant information can be found on the Render Foundation Website. If it is a good candidate for an RNP the admin or moderator will label it as an “Initial Proposal” and create a dedicated channel in the [Render Network Discord](https://discord.gg/rendertoken). Proposals must conform to the Render Network guidelines and use the RNP Template before approval by an admin or moderator. 
