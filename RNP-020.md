...
| ----- | --------------------- | -------- | ------------------ | ---------- |------- |
| 020 | Node Operator Forward Guidance Policy | Process | Pockets | 2025-10-28 | Defeated |
...
1. Take the rolling quarterly growth rate. (-15.13% in the last epoch of Fig 2)
2. Multiply it by the quarterly burns at that time. (12,125)
3. The result (12,125 x 0.849 = 10290) is then multiplied by the node change and min-max equal pain equations found in the appendix. ((10290 x 1.576) x 1.005 = 16,293) This determines the new anchor issuance target for the next quarter.
...
**Example**: In Fig 2 above, the 90 day TR%'s latest value is 14.88%. Avg daily volume was 376k USD, with an aggressiveness factor of 1.3 and a token price of 1.9 that gives a max cap of 96k RENDER, well above the current 15k policy level.
...
<p align="center"><sub><b>Figure 7 (Simulation with Actual Policy and Ceiling Overlaid)</b></sub></p>

The figure above illustrates a simulation of the proposed policy. Had it been implemented at the start of BME (initial issuance: 47.4k/epoch), the policy would have settled within three quarters. Following this stabilization period, issuance would have remained within a stable range, bounded between 27.6k (epoch 36) and 25.1k (epoch 99).
...
The actual policy is overlaid in figure 7. It shows how the proposed policy would have smoothed out the large cuts that happened in practice. The issuance ceiling is also shown, which would have been a limiting factor as early as epoch 2 and again at 8. 6 months before RNP-013's first cut. This demonstrates how the policy deals with a finite supply of deminishing emissions as well as maintains flexibility of emissions allocations without creating any extra emissions.

## Burn Growth Policy Simulation
![](RNP-020/media/image8.png)

<p align="center"><sub><b>Figure 8 (Burn Growth Policy Simulation)</b></sub></p>
...
![](RNP-020/media/image9.png)

<p align="center"><sub><b>Figure 9 (Burn Downturn Policy Simulation)</b></sub></p>
...
1. **Quarterly Burns**: 12,125 RENDER
2. **Growth Rate**: -15.13%
3. **Projected Burns**: 12,125 × 0.849 = 10,290 RENDER
4. **Node Adjustments**: Apply node change multipliers (1.576 and 1.005)
5. **Final Target**: 16,293 RENDER (before rebalance logic)
